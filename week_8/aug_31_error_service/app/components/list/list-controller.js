...
module.exports = (app) => {
  app.controller('ListController', ['$log', '$http', 'cfErrors', ListController]);
};

function ListController($log, $http, errors) {
  this.lists = [];
  this.getAllLists = function() {
    ...
    $http.get(this.baseUrl, this.config)
      .then(res => {
        ...
        errors.add(new Error('Network communication failure in get request for NoteList'));
        $log.error('error!', err);
      });
  };
